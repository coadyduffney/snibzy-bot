...
class OsrsCommand extends Commando.Command {
  ...
  async run(message, args) {
    ...
    if (newArgs[0] === "lookup") {
      if (!newArgs[1]) {
        message.reply(
          "Please provide me with a name to look up like this: `!osrs lookup snibzy`"
        );
      } else {
        let name = newArgs[1];
        // console.log(name);
          ...
          .then(player => {
            ...
            const embed = {
              ...
              fields: [
                ...
                {
                  name: "Clue Scrolls",
                  value: `**Easy**: ${clues.easy}  |  **Medium**: ${
                    clues.medium
                  }  |  **Hard**: ${clues.hard}  |  **Elite**: ${
                    clues.elite
                  }  |  **Master**: ${clues.master}`
                }
              ]
      ...
      if (!newArgs[1]) {
        message.reply(
          "Please provide me with an item to look up like this: `!osrs price item`"
        );
      } else {
        let str = [];
